...
const docsDir = path.join(root, '.gummy', 'docs');
const plansDir = path.join(root, '.gummy', 'plans');
let status = 0;
...
function validateContracts(file, storyId, specsById, storyDir) {
  ...
  if (raw.schema_version !== 'gummy.contracts.v1') {
    emit('invalid', `${file} schema_version must be gummy.contracts.v1.`);
  }
...
if (!exists(docsDir)) {
  emit('missing', `Expected Gummy docs directory: ${docsDir}`);
}

for (const doc of ['PRD.md', 'ARCHI.md', 'CONFIG.md']) {
  const file = path.join(docsDir, doc);
  if (!exists(file)) {
    emit('missing', `Gummy doc is missing: ${file}`);
  }
}

if (!exists(plansDir)) {
  emit('missing', `Expected Gummy story plans directory: ${plansDir}`);
} else {
...
if (status === 0) {
  console.log('- [ok] Gummy story state is mechanically valid.');
}
