...
function appendLesson(memoryDir, candidate) {
  ...
  if (!fs.existsSync(file)) {
    fs.writeFileSync(file, target === 'FAILURES.md' ? '# Gummy Failure Modes\n' : '# Gummy Memory\n');
  }
...
const memoryDir = path.join(os.homedir(), '.gummy', 'memory', slug);
...
if (action === 'list') {
  ...
  if (pending.length === 0) {
    console.log(`No pending Gummy memory candidates in ${candidatesFile}.`);
  } else {
    console.log(`Pending Gummy memory candidates in ${candidatesFile}:`);
...
if (risks.length > 0 && process.env.GUMMY_ALLOW_RISKY_MEMORY !== '1') {
...
candidate.verification.result = verificationResult(candidate.verification.command || 'scripts/check-gummy-state.sh .');
