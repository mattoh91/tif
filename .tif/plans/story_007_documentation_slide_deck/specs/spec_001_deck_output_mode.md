...
acceptance_checks:
  ...
  - id: check_002
    category: functional
    description: The rendered deck contains the required value-oriented sections sourced from tif artifacts.
    steps:
      ...
      - "Step 3: Verify each section's content traces back to a .tif/ source artifact."
    passes: true
  - id: check_003
    category: style
...
Deck section structure, sourced from existing tif artifacts:
...
Output to `docs/ppt/` as `deck.html`. Treat `docs/ppt/` as derived output regenerable from `.tif/` inputs.

## Research Findings

No external research required for the content mapping; inputs are tif's own artifacts. Styling is delegated to `ui-ux-pro-max`.
...
On a sample project, `/deck` produces a self-contained `docs/ppt/deck.html` with narrative, measurable-value, architecture, and per-component-with-research sections, each traceable to a `.tif/` artifact.
