...
# Gummy Product Requirements

## Problem Statement

Gummy is a personal SWE harness that should turn a user's plain-language build request into story-scoped ADRs, specs, contracts, implementation, documentation, and cleanup without redundant state files.

The harness should guide agents through one current path:

1. Capture product intent and stories in `PRD.md`.
2. Create one `.gummy/plans/story_<nnn>_<slug>/` folder per story.
...
| --- | --- | --- |
| story_001 | As a developer, I want Gummy to replace global completion state with story-local ADRs, specs, and contracts. | The state checker validates `.gummy/plans/story_*/ADR.md`, `contracts.json`, and `specs/spec_*.md`; story specs own completion. |
...
- No active Gummy instructions point agents to stale planning surfaces outside the current story-folder model.
- `scripts/check-gummy-state.sh .` validates story specs, contracts, ADRs, dependencies, and completion flags.
