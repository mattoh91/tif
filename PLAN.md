...
4. Two-tier testing: unit TDD stays as the implementer discipline (machine-internal), while each component/epic has an automated component acceptance gate backed by one or more functional checks.
...
The fork targets Claude Code and Codex. Claude Code packaging uses `.claude-plugin/`, `commands/`, `agents/`, and `hooks/`. Codex packaging uses `.codex-plugin/` with shared `skills/`, plus `.agents/plugins/marketplace.json` for local marketplace installation. The portable layer is `skills/` (markdown). Claude-style lifecycle behavior lives in `hooks/` and `commands/`; Codex should get shared skills plus skill-side fallbacks because Codex does not expose the same hook lifecycle.

### Resolved decisions

- **Doc mapping:** `PRD.md` = product/biz requirements (conventional). `FRD.md` + `ARD.md` + `CAVEATS.md` + `ARCHI.md` + `PRD.md` collectively = the spec set. `PLAN.md` = the implementation plan (component-by-component, with TDD nested inside each component). By default these files live under `.sweet/` or `docs/sweet/`; root-level output is a scaffold option for new app repos that want visible project-management files.
- **Test layering:** Two-tier. Unit TDD inside (RED-GREEN-REFACTOR, machine-internal). Each component/epic has an automated component acceptance gate backed by one or more functional checks. The gate verifies either DTO/data-contract shape across system/domain boundaries, or runs an automated e2e/user-flow scenario via tools such as Playwright, browser/computer-use automation, API scenario tests, or equivalent project-specific harness automation.
...
flowchart TD
    ...
    E --> F{Detect env vars:<br/>CURSOR_PLUGIN_ROOT /<br/>CLAUDE_PLUGIN_ROOT /<br/>COPILOT_CLI}
...
|---|-----------|-----------|----------------------------|--------------------|
| 1 | Spec doc structure (PRD/FRD/ARD/CAVEATS/ARCHI) and component-level functional acceptance, not unit-test review | UPDATE `brainstorming` to emit 5 separate spec files with mermaid C4 + sequence diagrams; UPDATE `writing-plans` to organize tasks by component/epic and require an automated component acceptance gate per task; KEEP TDD as nested implementer discipline | `skills/brainstorming/SKILL.md`, `skills/writing-plans/SKILL.md`, `skills/test-driven-development/SKILL.md` (add component-functional tier reference), `skills/requesting-code-review/SKILL.md` (review against component acceptance gate) | `skills/component-functional-testing/SKILL.md` (defines acceptable automated forms: (a) data-shape-at-boundary contract per OpenAI harness blog, (b) automated e2e/user-flow scenario via Playwright/browser/computer-use/API scenario tooling where applicable) |
...
| 3 | Failure-mode documentation & self-improvement | NEW skill for agent-facing failure-mode summarization; NEW `PreCompact` and `SessionEnd` hook scripts (Claude Code) that persist structured state. Hooks run shell scripts; they do not invoke markdown skills directly. On Codex, the agent invokes the skill explicitly because Codex has no equivalent hook lifecycle | `hooks/hooks.json` (register PreCompact, SessionEnd) | `skills/capturing-failure-modes/SKILL.md`, `hooks/pre-compact`, `hooks/session-end`, out-of-tree artifact `~/.sweet/memory/<project>/FAILURES.md` |
| 4 | Memory for fresh sessions | UPDATE `hooks/session-start` to auto-load recent state from `~/.sweet/memory/<project>/` on Claude-compatible hook hosts; UPDATE `finishing-a-development-branch` to write memory and update FRD progress; NEW `/preamble` slash command + skill that prints a copy-paste context block. Codex uses the skill-side generator/fallback rather than lifecycle hooks | `hooks/session-start`, `skills/finishing-a-development-branch/SKILL.md` | `skills/preamble/SKILL.md` (+ `commands/preamble.md` slash command where supported), `hooks/session-end`, `hooks/pre-compact`, out-of-tree artifacts under `~/.sweet/memory/<project>/`: `MEMORY.md`, `FAILURES.md`, `SESSIONS/<YYYY-MM-DD>.md` |
...
- `.sweet/PRD.md` or `docs/sweet/PRD.md` — product/biz requirements derived from your brainfart. The "what" and "why." Lists epics that map 1:1 to components/modules.
- `.sweet/FRD.md` or `docs/sweet/FRD.md` — functional requirements per epic. Each FRD epic ties to a component and includes an automated component acceptance gate definition: either a data-shape boundary contract or an automated e2e/user-flow scenario. Tracks per-epic completion status.
...
- `.sweet/PLAN.md` or `docs/sweet/PLAN.md` — implementation plan, organized by component/epic. Each component task lists: (i) the automated component acceptance gate, (ii) implementer-internal TDD step blocks. Replaces the date-prefixed `docs/sweet/plans/YYYY-MM-DD-feature.md` for Sweet-managed projects.
...
- **`skills/writing-plans/SKILL.md`** — re-frame plan as a list of components/epics matching FRD entries, each with its automated component acceptance gate and TDD step-blocks nested inside as implementer-internal detail. Output to `.sweet/PLAN.md` or `docs/sweet/PLAN.md` by default instead of dated `docs/sweet/plans/`.
- **`skills/test-driven-development/SKILL.md`** — keep the RED-GREEN-REFACTOR core; add an explicit reference to `component-functional-testing` as the layer above unit TDD. Document that unit tests are implementer-internal and the component acceptance gate is automated and component-level.
- **`skills/requesting-code-review/SKILL.md`** + **`agents/code-reviewer.md`** — review must verify the component acceptance gate exists, runs, and matches the FRD epic; not just unit tests.
- **`skills/finishing-a-development-branch/SKILL.md`** — on completion, append to `~/.sweet/memory/<project>/MEMORY.md` (design decisions, deviations from plan), tick FRD epic progress in the in-repo `FRD.md`, suggest `clear`/`compact` per the strategic-compact pattern (referenced from affaan-m/everything-claude-code).
- **`hooks/session-start`** — extend the existing bash script to additionally read `~/.sweet/memory/<project>/{MEMORY.md,FAILURES.md}` (resolved via git remote → project slug), last N git commits, and open FRD epics, then concat into the `additionalContext` JSON on hook-capable hosts. Preserve the polyglot env-var detection (`CLAUDE_PLUGIN_ROOT` / `CURSOR_PLUGIN_ROOT` / `COPILOT_CLI`).
...
- **`skills/component-functional-testing/SKILL.md`** — defines acceptable automated test forms: boundary data-shape contract per OpenAI's harness blog; automated e2e/user-flow scenario for frontend-involved components; API scenario tests or equivalent harness automation where appropriate. Cross-linked from `test-driven-development`, `writing-plans`, `requesting-code-review`.
...
- **`skills/preamble/SKILL.md`** + **`commands/preamble.md`** — `/preamble` slash command that prints a copy-paste context block: reads `~/.sweet/memory/<project>/MEMORY.md` + `FAILURES.md` + `SESSIONS/` (last N), last K git commits, open FRD epics, current PLAN.md tasks. Pure stdout — no file mutation.
...
- Single `docs/sweet/plans/YYYY-MM-DD-feature.md` (TDD-step granularity) → `.sweet/PLAN.md` or `docs/sweet/PLAN.md` (component/epic granularity matching FRD entries, with TDD step-blocks nested inside each component as implementer-internal detail).
- Review unit: TDD step → automated component acceptance gate pass.
...
- **Shared across Claude Code and Codex:** `skills/`, agent-facing instructions, artifact formats, component acceptance gate rules, and memory file format under `~/.sweet/memory/<project>/`.
- **Claude Code:** `hooks/` and `commands/` provide lifecycle automation.
- **Codex:** packages shared `skills/` through `.codex-plugin/plugin.json` and relies on explicit skill-side fallbacks. It should not depend on Claude hooks or slash commands being packaged or executed.
...
3. **Add `component-functional-testing` skill** + cross-link from `test-driven-development` and `requesting-code-review`. Pure addition.
...
5. **Refactor `writing-plans`** to component-level granularity with nested TDD; output to `PLAN.md`. Depends on (4).
...
7. **Update `finishing-a-development-branch`** to write MEMORY/FAILURES, tick FRD epic progress, suggest compact.
8. **Update `hooks/session-start`** to load MEMORY/FAILURES/recent commits/open FRD epics.
...
- **End-to-end on a real fork project:** create a fresh project, run `/scaffold` → brainstorm a small feature → verify 5 spec files written under `.sweet/` or `docs/sweet/` → verify PLAN.md is component-granular → execute one component → verify automated component acceptance gate runs and unit TDD nests inside → run `finishing-a-development-branch` → verify FRD epic ticked + MEMORY appended → start a new session with `/preamble` and confirm context restores.
...
- **Cross-harness sanity:** install in Claude Code AND Codex (via `~/.agents/skills/sweet` symlink). Verify SessionStart fires on Claude Code; verify Codex picks up skills via native discovery; verify the skill-side fallback for `capturing-failure-modes` works on Codex (agent invokes it explicitly since Codex has no hook lifecycle).
