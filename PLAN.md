# Customising the Cutiepie Fork into a Personal Cross-Harness SWE Harness

## Context

This fork (`mattoh91/cutiepie`) descends from `mattoh91/cutiepie`. Goal: turn it into a personal SWE harness that runs on both Claude Code and Codex, while:

1. Severing upstream contributor pressure (the inherited `CLAUDE.md` is anti-slop guidance for PRs *to* `mattoh91/cutiepie` — irrelevant here, actively misdirects agents working on the fork).
...
6. Adding failure-mode capture, memory persistence (out-of-tree, per-user, under `~/.cutiepie/memory/<project>/`), and a session-bootstrap `/preamble` skill that *generates a copy-paste string* (not auto-writes).
...
- **Agentic engineering guidelines:** `docs/cutiepie/AGENTIC_ENGINEERING_GUIDELINES.md` is the shared meta-contract for feature progress, component contracts, closed verification loops, YAGNI/KISS/DRY/SOLID, surgical changes, commit hygiene, memory updates, and subagent handoffs.
- **Memory location:** Per-user, out of tree, at `~/.cutiepie/memory/<project-slug>/{MEMORY.md,FAILURES.md,SESSIONS/}`. Never committed. The `<project-slug>` is derived from the repo's git remote or directory name.
...
## 1. The Cutiepie Agentic Flow (current state)

```mermaid
flowchart TD
    A[User starts session] --> B{Host platform?}
    B -->|Claude Code| C1[hooks/hooks.json<br/>SessionStart matcher: startup/clear/compact]
    B -->|Codex| C3[Native skill discovery via<br/>~/.agents/skills/cutiepie symlink]
    B -->|OpenCode| C4[.opencode/plugins/cutiepie.js<br/>system-prompt transform]
    ...
    F --> G[Read skills/using-cutiepie/SKILL.md]
    G --> H[Emit JSON in host-specific shape:<br/>hookSpecificOutput / additional_context / additionalContext]
    H --> I[using-cutiepie content<br/>injected as system context]
    C3 --> I
    C4 --> I
    ...
    O -->|writes| O1[(docs/cutiepie/specs/<br/>YYYY-MM-DD-topic-design.md)]
    O --> P
    P -->|writes| P1[(docs/cutiepie/plans/<br/>YYYY-MM-DD-feature.md)]
    P --> Q1
    P --> Q2
    ...
    U -.->|context lost| V[Next session: SessionStart re-injects<br/>using-cutiepie ONLY<br/>no memory of prior work]
...
| 3 | Failure-mode documentation & self-improvement | NEW skill for agent-facing failure-mode summarization; NEW hook scripts that persist structured state. Hooks run shell scripts; they do not invoke markdown skills directly. Agents invoke the skill explicitly when curated memory is needed | `hooks/hooks.json`, `.codex/hooks.json` | `skills/capturing-failure-modes/SKILL.md`, `hooks/pre-compact`, `hooks/session-end`, `hooks/codex-stop`, out-of-tree artifact `~/.cutiepie/memory/<project>/FAILURES.md` |
| 4 | Memory for fresh sessions | UPDATE `hooks/session-start` to auto-load recent state from `~/.cutiepie/memory/<project>/` on hook-capable hosts; UPDATE `finishing-a-development-branch` to write memory and update FRD progress; NEW `/preamble` slash command + skill that prints a copy-paste context block | `hooks/session-start`, `skills/finishing-a-development-branch/SKILL.md` | `skills/preamble/SKILL.md` (+ `commands/preamble.md` slash command where supported), `hooks/session-end`, `hooks/pre-compact`, out-of-tree artifacts under `~/.cutiepie/memory/<project>/`: `MEMORY.md`, `FAILURES.md`, `SESSIONS/<YYYY-MM-DD>.md` |
...
- `.cutiepie/PRD.md` or `docs/cutiepie/PRD.md` — product/biz requirements. The "what" and "why." Lists features/capabilities that map to FRD entries.
- `.cutiepie/FRD.md` or `docs/cutiepie/FRD.md` — functional requirements per feature, feature-to-component map, automated feature acceptance gates, component contract gates, and progress.
- `.cutiepie/ARD.md` or `docs/cutiepie/ARD.md` — architecture decision record. One section per significant decision (problem, options, decision, consequences).
- `.cutiepie/CAVEATS.md` or `docs/cutiepie/CAVEATS.md` — assumptions, dependencies, known unknowns, environmental constraints.
- `.cutiepie/ARCHI.md` or `docs/cutiepie/ARCHI.md` — mermaid diagrams: C4 L1 (system context), C4 L2 (containers), and per-component sequence diagrams.
- `.cutiepie/CONFIG.md` or `docs/cutiepie/CONFIG.md` — settings/config owner and documented hyperparameters/tunables: model names, temperatures, token limits, thresholds, retry counts, timeouts, polling intervals, batch sizes, feature flags, and similar values.
- `.cutiepie/PLAN.md` or `docs/cutiepie/PLAN.md` — implementation plan, organized by feature and component. Each feature lists its acceptance gate; each component lists its contract gate and implementer-internal TDD step blocks. Replaces the date-prefixed `docs/cutiepie/plans/YYYY-MM-DD-feature.md` for Cutiepie-managed projects.
...
**Out-of-tree (per-user, never committed) — at `~/.cutiepie/memory/<project-slug>/`:**
...
The `<project-slug>` resolution is git-remote-based (e.g. `github.com-mattoh91-cutiepie`) with directory-name fallback. The scaffolding-repo skill creates the `~/.cutiepie/memory/<project-slug>/` tree on first run.
...
- **`skills/brainstorming/SKILL.md`** — add (i) requirements-probing phase (sample data, edge cases, analogous-tool web research when available, local/user-provided fallback when not), (ii) emit `PRD.md` + `FRD.md` + `ARD.md` + `CAVEATS.md` + `ARCHI.md` + `CONFIG.md` multi-file output under `.cutiepie/` or `docs/cutiepie/` instead of single `design.md`, (iii) require mermaid C4 L1+L2 + sequence diagrams in `ARCHI.md`.
- **`skills/writing-plans/SKILL.md`** — re-frame plan as a list of features and components matching FRD entries, with feature acceptance gates, component contract gates, and TDD step-blocks nested inside as implementer-internal detail. Output to `.cutiepie/PLAN.md` or `docs/cutiepie/PLAN.md` by default instead of dated `docs/cutiepie/plans/`.
...
- **`skills/finishing-a-development-branch/SKILL.md`** — on completion, append to `~/.cutiepie/memory/<project>/MEMORY.md` (design decisions, deviations from plan), tick FRD feature/component progress in the in-repo `FRD.md`, suggest `clear`/`compact` per the strategic-compact pattern (referenced from affaan-m/everything-claude-code).
- **`hooks/session-start`** — extend the existing bash script to additionally read `~/.cutiepie/memory/<project>/{MEMORY.md,FAILURES.md}` (resolved via git remote → project slug), last N git commits, and open FRD features/components, then concat into the `additionalContext` JSON on hook-capable hosts.
...
- **`.claude-plugin/plugin.json`**, **`.codex-plugin/plugin.json`**, **`gemini-extension.json`**, **`.version-bump.json`** — rebrand `name`, `displayName`, `description`, `homepage`, `repository`, `author`. Suggest `cutiepie` as the fork name (the repo is already named `cutiepie`) so installs don't collide with upstream's `cutiepie`.
...
- **`skills/capturing-failure-modes/SKILL.md`** — agent-facing workflow for summarizing repeated patterns, root causes, and fixes. Hook scripts persist raw/structured session state; this skill turns that state into durable entries. Appends structured entries to `~/.cutiepie/memory/<project>/FAILURES.md`.
- **`skills/preamble/SKILL.md`** + **`commands/preamble.md`** — `/preamble` slash command that prints a copy-paste context block: reads `~/.cutiepie/memory/<project>/MEMORY.md` + `FAILURES.md` + `SESSIONS/` (last N), last K git commits, open FRD features/components, current PLAN.md tasks, and key DTO/schema/adapter references. Pure stdout — no file mutation.
- **`docs/cutiepie/AGENTIC_ENGINEERING_GUIDELINES.md`** — canonical engineering meta-contract referenced by repo-level instructions and skills.
- **`skills/scaffolding-repo/SKILL.md`** + **`skills/scaffolding-repo/template/`** — emits `init.sh`, `Makefile` (with `lint`, `typecheck`, `test`, `smoke`, `fmt` targets), CI baseline workflow (GitHub Actions by default), `.gitignore`, and the in-repo file skeletons under `.cutiepie/` or `docs/cutiepie/` by default: `PRD.md`, `FRD.md`, `ARD.md`, `CAVEATS.md`, `ARCHI.md`, `CONFIG.md`, `PLAN.md`. Also creates `~/.cutiepie/memory/<project>/{MEMORY.md,FAILURES.md,SESSIONS/}` on first run.
- **`hooks/pre-compact`** — bash script (polyglot like `session-start`) that persists current session state to `~/.cutiepie/memory/<project>/SESSIONS/<YYYY-MM-DD>.md` with a compaction marker. It cannot load a markdown skill directly.
...
- **In-repo artifact skeletons (templated by `scaffolding-repo` skill):** `.cutiepie/PRD.md`, `.cutiepie/FRD.md`, `.cutiepie/ARD.md`, `.cutiepie/CAVEATS.md`, `.cutiepie/ARCHI.md`, `.cutiepie/CONFIG.md`, `.cutiepie/PLAN.md` or `docs/cutiepie/*` equivalents.
- **Out-of-tree artifact skeletons (also templated by `scaffolding-repo`):** `~/.cutiepie/memory/<project>/{MEMORY.md, FAILURES.md, SESSIONS/}`.

### Displacements (one-for-one swaps)

- Single `docs/cutiepie/specs/YYYY-MM-DD-topic-design.md` → active in-repo set under `.cutiepie/` or `docs/cutiepie/`: `PRD.md` + `FRD.md` + `ARD.md` + `CAVEATS.md` + `ARCHI.md` + `CONFIG.md` (per-project, not date-prefixed; live-edited as the project evolves).
- Single `docs/cutiepie/plans/YYYY-MM-DD-feature.md` (TDD-step granularity) → `.cutiepie/PLAN.md` or `docs/cutiepie/PLAN.md` (feature/component granularity matching FRD entries, with TDD step-blocks nested inside each component as implementer-internal detail).
- Review unit: TDD step → component contract gate pass plus feature acceptance gate pass when the feature is integrated.
- Memory implicit-in-context-window → explicit out-of-tree memory at `~/.cutiepie/memory/<project>/`.
...
- The "PRs to mattoh91/cutiepie" workflow in `.github/PULL_REQUEST_TEMPLATE.md` — remove or rewrite for fork.
- `RELEASE-NOTES.md` upstream entries — keep as historical, but new entries are fork-scoped.

### What stays as-is

- `skills/using-cutiepie/`, `skills/using-git-worktrees/`, `skills/dispatching-parallel-agents/`, `skills/subagent-driven-development/`, `skills/executing-plans/`, `skills/systematic-debugging/`, `skills/verification-before-completion/`, `skills/writing-skills/`, `skills/receiving-code-review/`.
...
- **Shared across Claude Code and Codex:** `skills/`, agent-facing instructions, artifact formats, feature/component gate rules, guidelines, and memory file format under `~/.cutiepie/memory/<project>/`.
...
- `/Users/OHM02/Repos/cutiepie/CLAUDE.md`
- `/Users/OHM02/Repos/cutiepie/skills/brainstorming/SKILL.md`
- `/Users/OHM02/Repos/cutiepie/skills/writing-plans/SKILL.md`
- `/Users/OHM02/Repos/cutiepie/skills/test-driven-development/SKILL.md`
- `/Users/OHM02/Repos/cutiepie/skills/finishing-a-development-branch/SKILL.md`
- `/Users/OHM02/Repos/cutiepie/skills/requesting-code-review/SKILL.md`
- `/Users/OHM02/Repos/cutiepie/agents/code-reviewer.md`
- `/Users/OHM02/Repos/cutiepie/hooks/hooks.json`
- `/Users/OHM02/Repos/cutiepie/hooks/session-start`
- `/Users/OHM02/Repos/cutiepie/.claude-plugin/plugin.json`
- `/Users/OHM02/Repos/cutiepie/.codex-plugin/plugin.json`
- `/Users/OHM02/Repos/cutiepie/gemini-extension.json`
- `/Users/OHM02/Repos/cutiepie/.version-bump.json`
- `/Users/OHM02/Repos/cutiepie/README.md`
- `/Users/OHM02/Repos/cutiepie/scripts/sync-to-codex-plugin.sh`

## 6. New Files to Create

- `skills/component-functional-testing/SKILL.md`
- `docs/cutiepie/AGENTIC_ENGINEERING_GUIDELINES.md`
...
2. **Add `scaffolding-repo` skill** — produces the in-repo PRD/FRD/ARD/CAVEATS/ARCHI/PLAN skeleton under `.cutiepie/` or `docs/cutiepie/` + init.sh/Makefile/CI templates, plus the out-of-tree `~/.cutiepie/memory/<project>/` tree. Independent of skill graph changes.
...
- **Skill quality:** use `cutiepie:writing-skills` for adversarial pressure-testing of every changed skill. Compare before/after triggering on the same prompts.
- **End-to-end on a real fork project:** create a fresh project, run `/scaffold` → brainstorm a small feature → verify 5 spec files written under `.cutiepie/` or `docs/cutiepie/` → verify PLAN.md is feature/component-granular → execute one component → verify component contract gate runs and unit TDD nests inside → integrate feature → verify feature acceptance gate runs → run `finishing-a-development-branch` → verify FRD feature/component status ticked + MEMORY appended → start a new session with `/preamble` and confirm context restores.
