...
assert_contains "$repo_root/skills/story-planner/SKILL.md" ".gummy/plans/story_001_short_slug/" "story-planner defines story folder"
...
assert_contains "$repo_root/skills/contract-designer/SKILL.md" "scripts/check-gummy-state.sh ." "contract-designer runs state checker"
...
assert_contains "$repo_root/commands/socrates.md" "gummy:brainstorming" "socrates command routes to brainstorming"
assert_contains "$repo_root/commands/plato.md" "gummy:research-enrichment" "plato command routes to research"
assert_contains "$repo_root/commands/plato.md" "gummy:contract-designer" "plato command routes to contract designer"
assert_contains "$repo_root/commands/aristotle.md" "gummy:build" "aristotle command routes to build"
assert_contains "$repo_root/commands/aristotle.md" "Playwright" "aristotle command requires UI e2e evidence"
assert_contains "$repo_root/commands/finish.md" "gummy:update-state" "finish command refreshes state"
assert_contains "$repo_root/commands/finish.md" "gummy:memory-review" "finish command reviews memory candidates"
...
assert_contains "$repo_root/scripts/gummy-memory-review.sh" "GUMMY_ALLOW_RISKY_MEMORY" "memory review blocks risky promotion by default"
...
assert_contains "$repo_root/skills/cleanup/SKILL.md" "scripts/check-gummy-state.sh ." "cleanup runs state checker"
