...
using_method="$repo_root/skills/using-tif/SKILL.md"
guidelines="$repo_root/docs/tif/AGENTIC_ENGINEERING_GUIDELINES.md"
...
assert_contains "$repo_root/skills/using-tif/references/codex-tools.md" "multi-agent-adapter" "Codex tool mapping points to adapter"
assert_contains "$repo_root/README.md" "multi-agent-adapter" "README documents runtime adapter"

assert_contains "$using_method" ".tif/plans/story_<nnn>_<slug>/" "using-tif declares story folder state"
assert_contains "$using_method" "Spec frontmatter owns" "using-tif keeps completion state in specs"
assert_contains "$using_method" '`contracts.json` owns cross-spec schemas' "using-tif keeps contracts in story folder"
