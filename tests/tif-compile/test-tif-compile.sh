...
compiler="$repo_root/scripts/tif-compile.py"
tmpdir="$(mktemp -d)"
...
python3 "$compiler" "$tmpdir/source" --all --output "$tmpdir/out" >/tmp/tif-compile.log
...
assert_contains "$tmpdir/out/AGENTS.md" "<!-- tif-compile:example -->" "Codex section marker emitted"
assert_contains "$tmpdir/out/.cursor/rules/example.md" "alwaysApply: false" "Cursor rule is opt-in"

python3 "$compiler" "$tmpdir/source" --agent codex --output "$tmpdir/out" >/tmp/tif-compile-update.log
marker_count="$(grep -c '<!-- tif-compile:example -->' "$tmpdir/out/AGENTS.md")"
