#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the Tif plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/tif.js"
...
skill_count=$(find "$TIF_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $TIF_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-tif skill exists (critical for bootstrap)
echo "Test 3: Checking using-tif skill (required for bootstrap)..."
if [ -f "$TIF_SKILLS_DIR/using-tif/SKILL.md" ]; then
    echo "  [PASS] using-tif skill exists"
else
    echo "  [FAIL] using-tif skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$TIF_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/tif/' "$TIF_PLUGIN_FILE"; then
