#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the sweet plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/sweet.js"
...
# Test 3: Check using-sweet skill exists (critical for bootstrap)
echo "Test 3: Checking using-sweet skill (required for bootstrap)..."
if [ -f "$SUPERPOWERS_SKILLS_DIR/using-sweet/SKILL.md" ]; then
    echo "  [PASS] using-sweet skill exists"
else
    echo "  [FAIL] using-sweet skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/sweet/' "$SUPERPOWERS_PLUGIN_FILE"; then
