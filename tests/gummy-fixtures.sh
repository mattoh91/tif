...
create_gummy_docs_fixture() {
    local project_dir="$1"
    local story_dir="$project_dir/.gummy/plans/story_001_auth_system"

    mkdir -p "$project_dir/.gummy/docs" "$story_dir/specs"

    cat > "$project_dir/.gummy/docs/PRD.md" <<'EOF'
---
project_mode: POC
...
EOF
    ...
    cat > "$project_dir/.gummy/docs/ARCHI.md" <<'EOF'
...
EOF
    ...
    cat > "$project_dir/.gummy/docs/CONFIG.md" <<'EOF'
# Auth System Config
